...
const smsRowner = () => { return `「👑」 *Esta función solo puede ser usada por mi creador*\n\n> Antonio` }//NUMERO DE BOT
const smsOwner = () => { return `「👑」 *Esta función solo puede ser usada por mi creador*\n\n> Antonio` }//OWNER
const smsMods = () => { return `「⚜️」 *Esta función solo puede ser usada por mis desarrolladores.*` }//MODERADORES
const smsPremium = () => { return `「☁️」 *Esta función solo es para usuarios Premium.* PARA SER PREMIUM COMPRE UN PASE USANDO #pass premium` }//USUARIOS PREMIUM
const smsGroup = () => { return `「🐉」 *Esta funcion solo puede ser ejecutada en grupos*` }//PARA GRUPOS
const smsPrivate = () => { return `「🐉」 *Esta función solo puede ser usada en chat privado.*` }//AL PRIVADO
const smsAdmin = () => { return `「💨」 *Este comando solo puede ser usado por admins*` }//ADMINS
const smsBotAdmin = () => { return `「💨」 *Para usar esta función debo ser admin*` }//BOT CON ADMIN
const smsUnreg = () => { return `「🚨」 *¡Hey! no estas registrado, registrese para usar esta función*\n\n*/reg nombre edad*\n\n*_‼️ Ejemplo_* : */reg Antonio 1000*'` }//VERIFICAR
const smsRestrict = () => { return `「🐉」 *Esta característica esta desactivada.*` }//RESTRINGIR COMANDO
//MENU LISTA
...
const smsLista1 = () => { return `🌟 INFORMACIÓN DE AkumaBOT 🌟`}
...
const smsLista7 = () => { return `🐉 INSTALAR AkumaBOT 🐉`}
...
//_allantilink.js
const smsTextoYT = () => { return '🐉 𝗦𝘂𝗽𝗲𝗿 A k u m a𝗕𝗼𝘁-𝗠𝗗 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
const eAkumaCoins = () => { return '🐱 AkumaCoins' }
const eAkumaTickers = () => { return '🎫 Akuma Tickets' } 
...
const smsModP2 = () => { return `Habilitar función para que todos puedan usar AkumaBot`}
const smsModAd1 = () => { return `🛂 MODO ADMIN`}
const smsModAd2 = () => { return `Solo los Admins podrán usar AkumaBot en Grupos`}
...
const smsJBInfo2 = () => { return `💖 *La función es estable, sí presenta algún inconveniente Comuníquese al correo: arieldelarosas74@gmail.com*\n💝 *Puede hacer una Donación voluntaria por PayPal: ${global.paypal}*\n\n*Muchas Gracias por el apoyo a ${global.packname}*`}
...
const smsContacto4 = () => { return 'Contacto Oficial de AkumaBot 🐉'}
